...
{
    ui->setupUi(this);
    connect(ui->reg_btn,&QPushButton::clicked,this,&LoginDialog::switchRegister);//槽函数
...
void LoginDialog::initHttpHandlers()//注册登录的回调函数
{
    //注册获取登录回包逻辑
...
bool LoginDialog::enableBtn(bool enabled)//设置按钮是否可以点击触发，目前代码中没有设置为false，之后为了防止重复点击可以设置为flase
{
...
void LoginDialog::initHead()//设置窗口样式
{
    // 加载图片
...
void LoginDialog::slot_login_failed(int err){
    QString result = QString("登陆失败，err is %1")
                         .arg(err);
